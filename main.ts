...
serve(async (req) => {
    ...
    // 规则 1: 如果请求的是根路径，明确地服务 index.html
    // 这是修复 "Not Found" 问题的关键。
    if (pathname === "/") {
        return serveFile(req, "./index.html");
    }

    // 规则 2: 对于所有其他非 API 请求，让 serveDir 处理。
    // serveDir 会自动处理 /style.css, /script.js, 和 /static/ 目录下的所有文件。
    return serveDir(req, {
      fsRoot: ".", // 从项目根目录开始寻找文件
      urlRoot: "",  // URL路径直接映射到文件系统路径
    });
});
